...
describe('orcad migration source cutover persistence', () => {
  ...
  it('keeps a shutdown marker blocked while a source lease is non-final', () => {
    const { store } = setupSource({ dormantState: true, dormantSession: true })
    const worktreeId = 'repo-1::/srv/repo-1-worktree'
    store.setWorkspaceSession(
      {
  ...
  it('refuses to fence a source with a non-final PTY lease', () => {
    const { store, manifest } = setupSource()
    store.upsertSshRemotePtyLease({
      targetId: TARGET.id,
      ptyId: 'pty-live',
...
function addDormantAutomation(store: ReturnType<typeof createStore>, finalRun = true): string {
