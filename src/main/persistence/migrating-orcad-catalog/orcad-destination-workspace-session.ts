...
function assertNoSessionEntityConflicts(
  ...
  const existingOwners = sessionEntityOwners(existing)
  for (const [key, owner] of sessionEntityOwners(incoming)) {
    if (owner === '\0') {
...
function sessionEntityOwners(session: WorkspaceSessionState): Map<string, string> {
