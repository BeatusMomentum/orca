...
export function prepareOrcadMigrationDormantState(
  ...
  return {
    payload,
    newWorktreeMeta: selectNewKeyedRows(worktreeMeta, state.worktreeMeta, 'worktree_meta'),
...
function selectNewKeyedRows<T>(
  ...
  return incoming.filter((entry) => {
    ...
    assertSameValue(current, entry.value, `${label}:${entry.key}`)
    return false
  })
}
...
function assertKeyedRows<T>(
  ...
  for (const entry of incoming) {
    ...
    assertSameValue(current, entry.value, `receipt_dormant_mismatch:${label}:${entry.key}`)
  }
}
...
function sparsePresetKey(preset: Pick<SparsePreset, 'id' | 'repoId'>): string {
  return `${preset.repoId}\0${preset.id}`
}
