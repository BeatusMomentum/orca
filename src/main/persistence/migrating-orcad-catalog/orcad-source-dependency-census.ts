...
function collectDependencyCensus(
  ...
  counts['terminal-lease'] = state.sshRemotePtyLeases.filter(
    (lease) =>
      lease.targetId === scope.targetId && lease.state !== 'terminated' && lease.state !== 'expired'
  ).length
  ...
  counts['worktree-metadata'] = Object.entries(state.worktreeMeta).filter(
    ([ownerKey, meta]) =>
      orcadMigrationOwnerMatchesScope(ownerKey, scope) || meta.hostId === scope.hostId
  ).length
...
function countTerminalRecoveryState(
  ...
  // A consumer-owner row is only useful while a remote lease may still need to
  // be reclaimed. Relay teardown can leave the row behind after every lease has
  // reached a final state; retaining that tombstone must not strand a static
  // catalog migration. Any non-final lease remains a hard blocker below.
