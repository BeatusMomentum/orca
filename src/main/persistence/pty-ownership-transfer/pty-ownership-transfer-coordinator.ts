...
export class PtyOwnershipTransferCoordinator {
  ...
  async transfer(): Promise<PtyOwnershipTransferCoordinatorResult> {
    ...
    try {
      ...
      const attachment = await this.attachDestinationIfConfigured(
        destination,
  ...
  private async attachDestinationIfConfigured(
    destination: PtyOwnershipTransferDestinationAdapter,
  ...
  > {
    ...
    if (!capabilities || !this.options.source.attachDestination) {
      throw new Error('pty_ownership_transfer_destination_attachment_unconfigured')
    }
    const attachmentId = (this.options.createAttachmentId ?? randomUUID)()
    if (!attachmentId) {
    ...
    const disposeExitWatcher = this.options.source.onDestinationExitForAttachment
      ? this.options.destination.watchDestinationExitForAttachment(
          this.options.source,
          capabilities,
          this.options.identity,
          attachmentId,
      ...
      : this.options.source.onDestinationExit
        ? this.options.destination.watchDestinationExit(
            this.options.source,
            capabilities,
            destination,
            reservation
        ...
        : undefined
    const disposeOutputWatcher = this.options.source.onDestinationOutput
      ? this.options.destination.watchDestinationOutput(
          this.options.source,
          capabilities,
          this.options.identity,
          attachmentId,
    ...
    const disposeTransportLost = this.options.source.onDestinationTransportLost
      ? this.options.source.onDestinationTransportLost(
          capabilities,
          this.options.identity,
          attachmentId,
          () => {
      ...
      : undefined
    try {
      if (this.options.source.waitForDestinationStreamReady) {
        await this.options.source.waitForDestinationStreamReady(
          capabilities,
          this.options.identity,
          attachmentId
        )
      }
      const result = await this.options.source.attachDestination(
        {
          ...this.options.identity,
          version: PTY_OWNERSHIP_TRANSFER_WIRE_VERSION,
          attachmentId
      ...
      assertTransferIdentity(result, this.options.identity)
      this.options.destination.acceptDestinationAttachment(result, reservation)
      return {
        disposeExit: () => {
          disposeExitWatcher?.()
