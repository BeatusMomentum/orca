...
export class SshPtyOwnershipTransferRouteRegistry {
  ...
  write(ptyId: string, data: string, retry?: PtyProviderOperationRetry): boolean | null {
    ...
    const operationId = this.resolveCallerOperation(route, 'input', data, retry)
  ...
  retireWriteOperation(ptyId: string, operationId: string): Promise<boolean> | null {
    ...
    const operation = this.readCallerOperation(ptyId, operationId)
    if (!operation || operation.kind !== 'input') {
      return Promise.resolve(false)
    }
    this.assertCallerOperationRoute(operation, route)
  ...
  private resolveCallerOperation(
    route: ActiveRoute,
  ...
  private readCallerOperation(ptyId: string, operationId: string): CallerOperation | undefined {
    if (!operationId || operationId.length > 512) {
      return undefined
    }
  ...
  private assertCallerOperationRoute(operation: CallerOperation, route: ActiveRoute): void {
