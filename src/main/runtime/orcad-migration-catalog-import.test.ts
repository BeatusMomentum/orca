...
describe('durable orcad migration catalog import', () => {
  ...
  it('flushes a real abort but leaves already-absent and committed state read-only', async () => {
    ...
    await expect(abortStagedOrcadMigrationCatalogDurably(args)).resolves.toBe(aborted)
    await expect(abortStagedOrcadMigrationCatalogDurably(args)).resolves.toBe(unchanged)
