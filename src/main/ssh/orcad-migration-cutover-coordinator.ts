...
export async function stageOrcadMigrationDestination(args: {
  ...
  if (state.state === 'absent') {
    state = await resolveMutationResponse(
...
export async function commitOrcadMigrationDestination(args: {
  ...
  const remote = args.options?.remote ?? REMOTE_CATALOG_OPERATIONS
  const requestOptions = remoteRequestOptions(args.options)
  const state = await resolveMutationResponse(
...
async function resolveMutationResponse(
  ...
  try {
    ...
    try {
      ...
      // The mutation remains unverifiable; preserve its original failure and the source fence.
    }
    throw mutationError
  }
}
...
async function resolveAbortResponse(
  ...
  try {
    ...
    try {
      const observed = await read()
      if (observed.state === 'absent' || observed.state === 'committed') {
        return observed
      }
    } catch {
