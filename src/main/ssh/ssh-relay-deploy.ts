...
async function launchRelay(
  ...
  const ownershipTransferFlag = enableOwnershipTransferMutation
    ...
    : ''
  const launchCmd = `cd ${escapedDir} && chmod 600 ${shellEscape(credentialFile)} && nohup "${relayRuntime}" relay.js --detached --grace-time ${graceTime} --sock-path ${shellEscape(sockFile)}${endpointDirArg} --credential-file ${shellEscape(credentialFile)}${ownershipTransferFlag} --log-file ${shellEscape(logFile)} > ${shellEscape(logFile)} 2>&1 </dev/null &`
...
function windowsRelayLaunchCommand(
  ...
  const relayCommandLineParts = [
    ...
    ...(enableOwnershipTransferMutation
      ? [powerShellLiteral('--enable-ownership-transfer-mutation')]
      : []),
