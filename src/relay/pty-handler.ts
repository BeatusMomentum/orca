...
export class PtyHandler {
  ...
  resolveOwnershipTransferTerminal(terminalId: string): PtyOwnershipTransferTerminalSource | null {
    const managed = this.ptys.get(terminalId)
    if (!managed || managed.disposed) {
      return null
    }
    return Object.freeze({ terminalId, incarnationId: managed.incarnationId })
  }

  setOwnershipTransferInputFenced(terminalId: string, fenced: boolean): void {
    if (fenced) {
      if (!this.resolveOwnershipTransferTerminal(terminalId)) {
  ...
  private async loadPtyUncached(): Promise<RelayPtyModule | null> {
    ...
    const runtime = await loadRelayPtyRuntime({ skipNode: this.reloadPtyModuleFromDisk })
  ...
  private wireAndStore(managed: ManagedPty): void {
    ...
    managed.pty.onExit(({ exitCode }: { exitCode: number }) => {
      ...
      this.observeOwnershipTransferExit(managed.id, managed.incarnationId, exitCode)
  ...
  private observeOwnershipTransferOutput(
    ...
    try {
      return {
        fragments: this.ownershipTransferObserver?.observeOutput(id, data, emissionKey),
        failed: false
      }
    } catch (error) {
      process.stderr.write(
  ...
  private observeOwnershipTransferExit(id: string, incarnationId: string, code?: number): void {
    try {
  ...
  private enqueuePtyOutput(
    ...
    const canCoalesce =
      existing && !existing.transformed && !existing.ownershipTransfer && !meta.ownershipTransfer
  ...
  private flushPtyOutput(id: string, capturedQueue?: PendingPtyOutput[]): boolean {
    ...
    if (pending.ownershipTransferRetry) {
      ...
      if (observation.fragments && observation.fragments.length > 0) {
        ...
        this.pendingOutputByPty.set(id, queue)
        return this.flushPtyOutput(id, queue)
      }
    }
  ...
  private async attach(
    ...
    if (managed.pty.pid && !isProcessAlive(managed.pty.pid)) {
      ...
      this.notifyExitListener(managed)
      this.agentSessionOwners.release(managed.id)
      this.observeOwnershipTransferExit(managed.id, managed.incarnationId)
      disposeManagedPty(managed)
      this.removePty(id)
