...
/** Additive RPC contract used by a relay compatibility bridge.
 *
 * Older relays never register these methods and therefore continue using the existing
 * `pty.attach`/source-credit path.  A caller must probe the capability before issuing any
 * transfer method; unknown methods are not interpreted as a transfer failure.
 */
