...
function requireUnchangedEnvironment(
  ...
  if (
    !existing ||
    JSON.stringify(binding(existing, ignoreIntent)) !==
      JSON.stringify(binding(expected, ignoreIntent))
  ) {
    throw invalid(
...
function binding(environment: KnownRuntimeEnvironment, ignoreIntent: boolean): unknown {
  return {
