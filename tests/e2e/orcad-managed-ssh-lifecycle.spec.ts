...
async function waitForMarker(
  ...
  await expect
    .poll(
      async () =>
        JSON.stringify(
          await rpc(page, environmentId, 'terminal.read', { terminal: handle, limit: 1_000 })
        ),
      { timeout: 30_000 }
    )
    .toContain(marker)
...
test('renders lossless managed update, rollback, and guarded unlink over real SSH', async ({
  ...
  try {
    ...
    await expect(serverRow(section)).toContainText(
      'this server became the Active Server while it was stopping',
      { timeout: 30_000 }
    )
    await testInfo.attach('managed-orcad-interrupted-local-cleanup.png', {
      body: await orcaPage.screenshot(),
      contentType: 'image/png'
    })
